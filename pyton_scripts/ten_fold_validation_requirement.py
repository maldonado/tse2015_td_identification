...
all_requirement_debt_comments = 0 
...
print(len(all_without_classification_comments))
...
cursor.execute("select classification, treated_commenttext from processed_comment where classification in ('IMPLEMENTATION') and treated_commenttext != ''")
all_requirement_debt_comments = cursor.fetchall()
# shuffle the results so the results will not be biased per project (due to the sequential insertion of the files)
random.shuffle(all_requirement_debt_comments, random.random)
all_requirement_debt_comments_remainder = len(all_requirement_debt_comments) % 10
...
for x in range (0,10):
    temp_list = []
    for requirement_debt_counter in range (0,75):
        temp_list.append(all_requirement_debt_comments.pop())
    ...
    if x == 9 : 
        for requirement_debt_counter in range (0, all_requirement_debt_comments_remainder):
            temp_list.append(all_requirement_debt_comments.pop())
...
for testing_part in execution_order.keys():
    ...
    current_classification_path = "/Users/evermal/git/tse2015/tse2015_td_identification/datasets/requirement/ten_folds_validation/"+ str(testing_part)+"/"
