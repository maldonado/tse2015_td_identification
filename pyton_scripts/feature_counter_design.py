...
design_folder_path = '/Users/evermal/git/tse2015/npl_tools/datasets/design_vs_without_classification/filtered_top_features'

positive_feature_regex = ".*1-SW-(.*),DESIGN\)\s*(\d\.\d\d\d\d)"
negative_feature_regex = ".*1-SW-(.*),DESIGN\)\s*\-(\d\.\d\d\d\d)"
all_feature_regex = ".*1-SW-(.*),DESIGN.*(\d\.\d\d\d\d)"

feature_average = 0
for root, dirs, files in os.walk(design_folder_path):
    for f in files:
