...
\begin{abstract}
During the development of a software system developers face unpredictable difficulties, and in many cases they apply unconventional solutions such as hacks and workarounds. The \textit{metaphor} of \textit{technical debt} was introduced to express this trade off between productivity and quality. More recently, our work has shown that it is possible to detect technical debt using source code comments (i.e., self-admitted technical debt), and that the most common types of self-admitted technical debt are design and requirement debt. However, this approach heavily depends on the manual classification of source code comments. In this paper we present an approach to automatically identify design and requirement \SATD using a Natural Language Processing (NLP) based approach. We studied ten open source projects: Ant, ArgoUML, Columba, EMF, Hibernate, JEdit, JFreeChart, Jmeter, JRuby and SQuirrel SQL. We find that the average F1 measure achieved when identifying \SATD significantly outperforms the current state-of-the-art techniques. Moreover, we evaluate and present the top 10 words that indicate \SATD. We find that for design debt, words related to sloppy or mediocre source code are the best indicators, whereas for requirement debt, words related to enhancing or completing tasks are the best indicators. Lastly, we examine the size of the training data needed for our NLP-based approach and find that training datasets as small as 1,444 design debt comments and 380 requirement debt comments are enough to effectively identify \SATD comments. To facilitate future work we make our datasets of classified comments publicly available, which are composed of more than 62,000 manually classified comments. 
\end{abstract}
