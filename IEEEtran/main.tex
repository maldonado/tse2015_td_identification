...
\author{Everton~da S.~Maldonado,
        Nikolaos~Tsantalis,~\IEEEmembership{Member,~IEEE,}
        and~Emad~Shihab,~\IEEEmembership{Member,~IEEE,}% <-this % stops a space
\IEEEcompsocitemizethanks{\IEEEcompsocthanksitem E. da S. Maldonado, N. Tsantalis and E. Shihab are with the Department
...
\nikos{The evil reviewer might say that we also do some manual inspection in the ``manual classification'' phase}
...
\nikos{I would expect here to see... In contrast to the comment pattern approach~\cite{Potdar2014ICSME}, which requires a continuous manual inspection of comments, the classifier needs to be trained only once... I guess this is not completely true, but we have to say something}
