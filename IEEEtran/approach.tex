...
% by Potdar and Shihab \cite{Potdar2014ICSME}
...
\revised{ 
Below, we define design and requirement \SATD that we were able to detect and provide example comments to help the reader grasp the different types of self-admitted technical debt comments.

\vspace{1mm}
\noindent\textbf{Self-admitted design debt:} These comments indicate that there is a problem with the design of the code. They can be comments about misplaced code, lack of abstraction, long methods, poor implementation, workarounds or a temporary solution. Usually these kinds of issues are resolved through refactoring (i.e., restructuring of \emph{existing code}), or by re-implementing \emph{existing} code to make it faster, more secure, more stable and so forth. Lets consider the following comments:
...
\vspace{1mm}
\noindent\textbf{Self-admitted requirement debt:} These comments convey the idea that the implementation of a requirement is not complete.  In general, requirement debt comments express incompleteness of the method, class or program showing that the due code to implement a requirement was not done yet (i.e., \emph{inexistent code}) as it can be observed in the following comments:
