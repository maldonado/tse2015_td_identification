...
\revised{To execute the second experiment, we shuffle the comments from all projects into a big dataset. Then, we split this dataset into 10 identical parts making sure that each part has an identical proportion of \SATD and without technical debt comments. Next, we use one of the ten parts to test the max entropy classifier and the other 9 to train it. The training data is incrementally feed into the classifier by batches of 100 comments at time, and we also make sure that each batch of 100 comments contains the same proportion of \SATD and without technical debt comments. We repeated this process for each one of the 10 dataset parts and report on our findings.
...
\revised{\noindent \textbf{Per-comment based experiment results - design debt:} Figure~\ref{fig:design_debt_per_comment_average} shows the ten fold average F1-measure obtained while identifying design \SATD. We find that the maximum F1-measure improves as we increase the number of comments, achieving its highest value with \todo{x} amount of comments. However, the most accentuated improvement in the F1-measure performance happens within the first 10k comments, more precisely 80\% and 90\% of the maximum F1-measure is achieved with 3900 and 9700 comments in the training dataset. In this experiment we added 100 comments per iteration, and for each batch of 100 comments we keep the same ratio between design \SATD and without technical debt comments of the training dataset which is approximatively 5\% (i.e.,\todo{} 2700/58120). Therefore, we used 195 and 485 design \SATD comments to achieve 80\% and 90\% of the maximum F1-measure respectively. This amounts to a reduction of \todo{divide total number of design debt} and \todo{} in training data to achieve 80\% and 90\% of the maximum F1-measure.

The number of design \SATD comments needed to achieve 80\% and 90\% of the maximum F1-measure in our second experiment is considerable lower than the the number of \SATD comments necessary to achieve similar marks on the first experiment. One possible reason is that in the second experiment (i.e., per-comment based) comments that belongs to same project are used as training data for the maximum entropy classifier.}{R2-14}
...
\revised{\noindent \textbf{Per-project based experiment results - requirement debt:}}{R2-14}
